...
//		if(strcmp(tran_str.ChangeStr(node->getNodeValue()), "#text") != 0)
		...
		printf("NODE: %s\n", /*tran_str.ChangeStr(*/XMLString::transcode(node->getNodeName()));
	GetChilds(node);
	return;
}
...
int main(int argc, char* argv[])
{
	...
	//读入一个文档
	DOMDocument* doc = dbf->parseURI(XMLString::transcode("TestModuleDeclare2.xml"));//TestInfoBase.xml
...
DOMNodeList* map1 = root->getElementsByTagName(XMLString::transcode("ProviderID"));
printf("%d\n", map1->getLength());
	node = node->getNextSibling();
printf("%s\n", XMLString::transcode(node->getNodeName()));
//node->getAttribute(XMLString::transcode("Name"));
DOMNamedNodeMap* map = node->getAttributes();
DOMNode* node11 = map->getNamedItem(XMLString::transcode("Name"));
printf("%d\n", map->getLength());
printf("aaaaaaaaaaaaaaa:%s\n", XMLString::transcode(node11->getNodeValue()));
