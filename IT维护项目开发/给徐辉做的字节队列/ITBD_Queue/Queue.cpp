...
typedef   struct   tagSMM_node
	{		
		SMM_INT       iDataBlockSize;                    //数据块大小
		SMM_UCHAR	  *Data_Block;			    //队列结点数据块指针
		tagSMM_node   *pNext;					//下一个数据块指针
	}SMM_node; 
...
typedef   struct  tagSMM_Queue_Info
	{
		SMM_INT	    iID;						//队列ID
		SMM_INT		iSize;						//队列长度
		SMM_INT     iAvailable;                 //可用的(即剩余)队列长度
		SMM_INT		iMode;						//判断阻塞或非阻塞
		tagSMM_node * pFront;					//队列读指针
		tagSMM_node * pRear;
...
SMM_BOOL ITDQ_CreateQueue(SMM_UINT uiMaxLen, tagSMM_node* & pNode)			//创建队列函数
{
	...
	p1 = pNode;
	for(SMM_UINT i = 0; i < uiMaxLen-1; i++)
	{
...
{	
	...
	ID = p_Info->iID;   //把调用ITDQ_Init函数赋值的ID号赋值给局部变量ID
	ID = ID + 1;        //ID加1
	p_Info->pNext = (tagSMM_Queue_Info*)malloc(sizeof(tagSMM_Queue_Info));
	p_Info = p_Info->pNext;
	p_Info->hSemWrite = CreateSemaphore(NULL,1,1,NULL);			//创建写信号量
	p_Info->hSemRead = CreateSemaphore(NULL,0,1,NULL);			//创建读信号量
	p_Info->iID = ID;
...
***********************************************************/
SMM_INT ITDQ_Read( SMM_INT iId, SMM_UCHAR pData[][Data_Size], SMM_INT iNum)
{	
		...
		/*	for(i =0 ;i < iNum; i++)
			{
				for(j = 0; j < Data_Size; j++)					//判断读取	
		         pData[i][j] = p_Info->pFront->cData[j];
...
***********************************************************/
SMM_INT ITDQ_Write(SMM_INT iId, SMM_UCHAR* pData, SMM_INT iNum)
{
	tagSMM_Queue_Info *p_Info = &pQueue_Info;
	tagSMM_node* p_Node = 0;
	int i = 0;
	...
	if(p_Info->iMode == ITDQ_BLOCK_YES)							//同步读写操作
	{
		if(p_Info != NULL)										//找到值为iId的队列
		{
			...
			for(i = 0; i < iNum; i++)
			{
				for(j = 0; j < Data_Size; j++)						//判断读取	
				{
					p_Info->pRear->cData[j] =  pData[i][j];
				}
