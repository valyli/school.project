...
//从注册表获取注册用户姓名信息
bool Get_RegisteredOwner(CManageData& Data)
{
	CString str_owner;
...
//从注册表获取注册组织信息
bool Get_RegisteredOrganization(CManageData& Data)
{
	CString str_company;
	HKEY hKEY;//定义有关的 hKEY, 在查询结束时要关闭。
	LPCTSTR data_Set;
...
//设置RegisteredOwner
bool Set_RegisteredOwner(CManageData& Data)
{
	CString str_owner;
	Data.GetData(str_owner);
...
bool Set_RegisteredOrganization(CManageData& Data)
{
	CString str_company;
	Data.GetData(str_company);
	//定义有关的 hKEY, 在程序的最后要关闭。
	HKEY hKEY;
	LPCTSTR data_Set;
