...
@备注
---------------------------------------------------------------------------------------------*/
bool DetectBIOSID_DetectHardwareModule(CManageData& Data)
{
	char buff[256];
	if(!GetBiosId(buff))
		return false;
	CString BIOS_ID = buff;
	Data.SetData(BIOS_ID);
	return true;
}


bool DetectBIOSType_DetectHardwareModule(CManageData& Data)
{
	char buff[256];
	if(!GetBiosType(buff))
		return false;
	CString BIOS_Type = buff;
	Data.SetData(BIOS_Type);
	return true;
}


bool DetectBIOSDate_DetectHardwareModule(CManageData& Data)
{
	char buff[256];
	if(!GetBiosDate(buff))
		return false;
	CString BIOS_Date = buff;
	Data.SetData(BIOS_Date);
	return true;
}
