...
void CClientTransferModuleDemoDlg::OnBUTTONTransferModule() 
{
	...
	lpfn = (bool (__cdecl *)(struct ModuleInfomationListStruct &,class CManageData &))GetProcAddress(hLibrary, "ClientTransferModule");
	if(lpfn == NULL)
	{
		AlarmErrorMessage("Get Function ClientTransferModule() Failed", "Load Library Error");
		return;
	}
