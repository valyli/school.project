...
bool CModuleInfoList::DisplayChild(IXMLDOMNodePtr pChild)
{
	...
	if(pChild->nodeType == NODE_TEXT)
	{
		...
		if(temp == "InfoModuleTypeProviderId")
		{
			m_InfoList.InfoModuleTypeProviderId = atoi(pChild->text);
		}
		else if(temp == "InfoModuleTypeId")
		{
			m_InfoList.InfoModuleTypeId = atoi(pChild->text);
		}
		else if(temp == "InfoModuleProviderId")
		{
			m_InfoList.InfoModuleProviderId = atoi(pChild->text);
		}
		else if(temp == "InfoServiceProviderId")
		{
			m_InfoList.InfoServiceProviderId = atoi(pChild->text);
		}
		else if(temp == "NameBaseId")
		{
			m_InfoList.NameBaseId = atoi(pChild->text);
		}
		else if(temp == "OperationID")
		{
	...
	else if(pChild->nodeType == NODE_ELEMENT)
	{
		...
		if(pChildLast != NULL)
		{
			...
			if(temp == "FunctionSection" && temp2 != "NameBaseId")
			{
				m_InfoList.ListArray.Add(Info);
			}
		}
...
//获取模块信息的公用接口函数
bool CModuleInfoList::GetInfoList(ModuleInfomationStruct &InfoList)
{
	::CoInitialize(NULL);
...
//			AfxMessageBox(m_plDomDocument->xml);
		...
		if(DisplayChildren(m_pDocRoot))
		{
			...
			m_InfoList.ListArray.Add(Info);
			//将数据复制给返回参数
			InfoList.InfoModuleTypeProviderId = m_InfoList.InfoModuleTypeProviderId;
			InfoList.InfoModuleTypeId = m_InfoList.InfoModuleTypeId;
			InfoList.InfoModuleProviderId = m_InfoList.InfoModuleProviderId;
			InfoList.InfoServiceProviderId = m_InfoList.InfoServiceProviderId;
			InfoList.NameBaseId = m_InfoList.NameBaseId;
			for(int i = 0; i < m_InfoList.ListArray.GetSize(); i++)
				InfoList.ListArray.Add(m_InfoList.ListArray.GetAt(i));
