...
void CTestGetInfoFromDllDlg::OnButtonGetinfo() 
{
	...
	CArrayModuleInfomationListStruct m_Info;
	...
	if(m_CFileDialog.DoModal() == IDOK)
	{
		...
		for(int i = 0; i < m_Info.GetSize(); i++)
		{
			...
			temp += "\nOperationID: ";
			temp += itoa(m_Info.GetAt(i).OperationID, buff, 10);
			
			temp += "\nInfomationElementTableID: ";
			temp += itoa(m_Info.GetAt(i).InfomationElementTableID, buff, 10);
			
			temp += "\nInfomationElementID: ";
			temp += itoa(m_Info.GetAt(i).InfomationElementID, buff, 10);
			
			temp += "\nOperationType: ";
			temp += itoa(m_Info.GetAt(i).OperationType, buff, 10);
			
			temp += "\nServiceFileName: ";
			temp += m_Info.GetAt(i).ServiceFileName;
			
			temp += "\nSupportSystemType: ";
			temp += itoa(m_Info.GetAt(i).SupportSystemType, buff, 10);
			
			temp += "\nModuleType: ";
			temp += itoa(m_Info.GetAt(i).ModuleType, buff, 10);
			
			temp += "\nInterfaceFunction: ";
			temp += m_Info.GetAt(i).InterfaceFunction;
			
			temp += "\nParameterType: ";
			temp += itoa(m_Info.GetAt(i).ParameterType, buff, 10);
			
			temp += "\nIsStorageReturnValueInParameter: ";
			temp += itoa(m_Info.GetAt(i).IsStorageReturnValueInParameter, buff, 10);
			
			temp += "\nParameterValue: ";
			temp += m_Info.GetAt(i).ParameterValue;
...
bool CTestGetInfoFromDllDlg::GetInfoListFromDll(CArrayModuleInfomationListStruct &m_Info,
												CString strDllFilePath)
{
	CString temp;
	
	bool (FAR *lpfn)(CArrayModuleInfomationListStruct &InfoListArray);
	...
	lpfn = (bool (__cdecl *)(CArrayModuleInfomationListStruct&))GetProcAddress(hLibrary, "InfoList");
	if(lpfn == NULL)
	{
...
void CTestGetInfoFromDllDlg::OnButtonAddInfomationFromDllToMemory() 
{
	//选择要查看的dll文件
	CString strDllFilePath;
	CArrayModuleInfomationListStruct m_InfoCArray;
	...
	if(m_CFileDialog.DoModal() == IDOK)
	{
		...
		if(!GetInfoListFromDll(m_InfoCArray, strDllFilePath))
		{
			AlarmErrorMessage("GetInfoListFromDll() Failed", "");
			return;
		}
		int SizeOfArray;
		for(int i = 0; i < m_InfoCArray.GetSize(); i++)
		{
			m_InfoStruct = m_InfoCArray.GetAt(i);
