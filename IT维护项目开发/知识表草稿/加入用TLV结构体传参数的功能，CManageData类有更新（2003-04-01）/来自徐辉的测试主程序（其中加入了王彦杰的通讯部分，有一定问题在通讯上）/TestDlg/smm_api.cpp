...
{   
	...
	//(9)设置加密算法列表
    if (SSL_CTX_set_cipher_list(ctx, CIPHER_LIST_SERVER) != 1)
	{
        return 5 ;
	}
   return 0 ; //设置成功
...
SMM_VOID SC_freearray(SMM_INT iSocket)
{
	...
	//(2)后续元素前移
	for (i = iSocket+1 ;i<SOCKET_MAX;i++)  //遍历数组
	{   
		if (stSSLSocket[j].flag != -1)
		{
			stSSLSocket[j].ssl =  stSSLSocket[i].ssl  ; 
			stSSLSocket[j].ctx  = stSSLSocket[i].ctx  ;
			stSSLSocket[j].s   =  stSSLSocket[i].s;
			stSSLSocket[j].sock_serv = stSSLSocket[i].sock_serv ;
			stSSLSocket[j].flag = stSSLSocket[i].flag ; 
		}
		else   //-1:表示已经到数组结尾
		{
			break ;
						...
						,SMM_ULONG ulServer,SMM_ULONG ulVersion)
{
	...
	if (ulAuth == SC_VERIFY_YES)                      //采用认证模式
	{
	   ...
	   {    
	        ...
	        //SSL_CTX_set_cipher(stSSLSocket[iSocket].ctx,pstCipher) ;
	   }
	   else
	   {
...
SMM_INT SC_accept(SMM_INT iSocket)
{
	...
	//(1)在数组中查找套接字对应的下标
	iSock_serv = SC_queryarray(iSocket) ; 
	if (iSock_serv == -1)
	{
		return -7 ;
	}
	//(2)sock_serv:-2:表示该数组结构中该子项没有使用,但s已经使用
	if (stSSLSocket[iSock_serv].sock_serv == -1) //－1:表示还没有分配空间
	{
        iSock_serv  = SC_mallocarray() ;
        switch (iSock_serv)
		{
			case -1:      //分配内存失败
				 return -2 ;
    ...
    if (stSSLSocket[iSock_serv].ssl != NULL)       //使用SSL安全通讯
	{   
		//(3-1)等待客户端socket连接请求
		stSSLSocket[iSock_serv].sock_serv = accept(stSSLSocket[iSock_serv].s
			,(struct sockaddr *) &stClient_S,&iClient) ;
		if (stSSLSocket[iSock_serv].sock_serv == SOCKET_ERROR)
		{
			return -4 ;
		}
		//printf("sock_serv=%d",stSSLSocket[iSock_serv].sock_serv) ; //测试accept返回值
        //(3-2)绑定socket和ssl
        iError = SSL_set_fd(stSSLSocket[iSock_serv].ssl,stSSLSocket[iSock_serv].sock_serv);
		if (iError<0)
		{
			return -5 ;
		}
        //(3-3)等待客户端ssl连接请求
		iError = SSL_accept(stSSLSocket[iSock_serv].ssl) ;
		//printf("ssl_accept=%d",iError) ; //测试SSL_accept返回值
		if (iError <0 )
		{   
			SSL_shutdown(stSSLSocket[iSock_serv].ssl) ; 
			return -6 ;
		}
	}
	else   //不使用SSL通讯
...
SMM_INT SC_ioctl(SMM_INT iSocket,SMM_LONG lCmd,SMM_ULONG pArgp)
	{
		...
		switch(lCmd)			//根据用户输入的命令参数进行定义，只考虑阻塞/非阻塞
		{
			case FIONBIO:      //设置阻塞/非阻塞模式
				iErr = ioctlsocket(stSSLSocket[0].s,FIONBIO,&ulBlock);
				break;
			default:          //其他模式暂时不处理
				break;
		}
...
SMM_INT SC_close(SMM_INT iSocket,SMM_INT iHow,SMM_INT iMain)
	{
        ...
        //(2)关闭socket/ssl
		if(stSSLSocket[iSSL].ssl == NULL) //不使用SSL通讯
		{   
			switch (iHow)
			{
				 case SD_SEND:
...
//写共享数据区
bool SetTransfer_C2S_StructtoMapFileC2S(tagSMM_c2s *m_Data_Transfer_C2S_Struct)
{
	//获取共享数据区
...
//写共享数据区
bool SetTransfer_S2C_StructtoMapFileS2C(tagSMM_s2c m_Data_Transfer_S2C_Struct)
{
	//获取共享数据区
...
//C2S(客户端到服务器:暂时不用)
//从共享数据区获取TLVStruct
bool GetTransfer_C2S_StructfromMapFileC2S( tagSMM_c2s *m_Data_Transfer_C2S_Struct)
{
	//获取共享数据区
	hMapFileC2S = OpenFileMapping(FILE_MAP_WRITE, TRUE, "ServerJNITLVModuleC2S");
	if(hMapFileC2S == NULL)
...
//S2C(服务器到客户端)
//从共享数据区获取TLVStruct
bool GetTransfer_S2C_StructfromMapFileS2C(tagSMM_s2c *m_Data_Transfer_S2C_Struct)
{
	//获取共享数据区
	hMapFileS2C = OpenFileMapping(FILE_MAP_WRITE, TRUE, "ServerJNITLVModuleS2C");
	if(hMapFileS2C == NULL)
	...
	for(int i = 0; i < sizeof(Transfer_S2C_Struct); i++)
    {
    	memcpy(m_Data_Transfer_S2C_Struct,lpMapAddress,sizeof(tagSMM_s2c)) ;
