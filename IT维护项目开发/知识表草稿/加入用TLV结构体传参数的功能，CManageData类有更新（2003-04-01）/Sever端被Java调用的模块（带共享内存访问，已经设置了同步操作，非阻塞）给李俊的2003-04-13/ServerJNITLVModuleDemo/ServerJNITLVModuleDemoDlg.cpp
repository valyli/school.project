...
void CServerJNITLVModuleDemoDlg::OnButtonSetTlv() 
{
    ...
    //获取函数句柄
    bool (FAR *lpfn)(tagSMM_c2s TLVData);
	lpfn = (bool (__cdecl *)(struct tagSMM_c2s))GetProcAddress(hLibrary, "Set_C2S");
	if(lpfn == NULL)
	{
		AlarmErrorMessage("Get Function Set_C2S() Failed", "Load Library Error");
    ...
    //设置数据
    tagSMM_c2s Data;
    Data.m_Data_Transfer_C2S_Struct.Value.Type = CString_Value;
    Data.m_Data_Transfer_C2S_Struct.Value.Length = 255;
	UpdateData(TRUE);
    strcpy(Data.m_Data_Transfer_C2S_Struct.Value.Value.Value_char, this->m_TLV);

    //调用函数
	bool flag = (*lpfn)(Data);
	if(flag)
	{
		::MessageBox(0, "Set_C2S() Success", "CServerJNITLVModuleDemoDlg", MB_OK);
		this->m_TLV = "";
		UpdateData(FALSE);
	}
}
...
void CServerJNITLVModuleDemoDlg::OnButtonGetNewDataFromMemoryToDll() 
{
    ...
    //获取函数句柄
    bool (FAR *lpfn)(tagSMM_c2s& TLVData);
	lpfn = (bool (__cdecl *)(struct tagSMM_c2s&))GetProcAddress(hLibrary, "Get_C2S");
	if(lpfn == NULL)
	{
		AlarmErrorMessage("Get Function Get_C2S() Failed", "Load Library Error");
		FreeLibrary(hLibrary);
		return;
	}
	...
	if(flag)
	{
		if(Data.m_Data_Transfer_C2S_Struct.Value.Type == CString_Value)
		{
			this->m_TLV = Data.m_Data_Transfer_C2S_Struct.Value.Value.Value_char;
			UpdateData(FALSE);
		}
		::MessageBox(0, "Get_C2S() Success", "CServerJNITLVModuleDemoDlg", MB_OK);
	}
}
...
void CServerJNITLVModuleDemoDlg::OnButtonSetTlv2() 
{
    ...
    //获取函数句柄
    bool (FAR *lpfn)(tagSMM_s2c TLVData);
	lpfn = (bool (__cdecl *)(struct tagSMM_s2c))GetProcAddress(hLibrary, "Set_S2C");
	if(lpfn == NULL)
	{
		AlarmErrorMessage("Get Function Set_S2C() Failed", "Load Library Error");
		FreeLibrary(hLibrary);
		return;
	}
    ...
    //设置数据
    tagSMM_s2c Data;
	Data.m_Data_Transfer_S2C_Struct.InfoBaseID.InfomationElementTableID = 1;
	Data.m_Data_Transfer_S2C_Struct.InfoBaseID.InfomationElementID = 1;
    Data.m_Data_Transfer_S2C_Struct.Value.Type = CString_Value;
    Data.m_Data_Transfer_S2C_Struct.Value.Length = 255;
	UpdateData(TRUE);
    strcpy(Data.m_Data_Transfer_S2C_Struct.Value.Value.Value_char, this->m_Test);
...
void CServerJNITLVModuleDemoDlg::OnButtonGetNewDataFromMemoryToDll2() 
{
    ...
    //获取函数句柄
    bool (FAR *lpfn)(tagSMM_s2c& TLVData);
	lpfn = (bool (__cdecl *)(struct tagSMM_s2c&))GetProcAddress(hLibrary, "Get_S2C");
	if(lpfn == NULL)
	{
		AlarmErrorMessage("Get Function Get_S2C() Failed", "Load Library Error");
		FreeLibrary(hLibrary);
		return;
	}
	...
	if(flag)
	{
		if(Data.m_Data_Transfer_S2C_Struct.Value.Type == CString_Value)
		{
			this->m_Test = Data.m_Data_Transfer_S2C_Struct.Value.Value.Value_char;
			UpdateData(FALSE);
		}
		::MessageBox(0, "Get_S2C() Success", "CServerJNITLVModuleDemoDlg", MB_OK);
	}
}
