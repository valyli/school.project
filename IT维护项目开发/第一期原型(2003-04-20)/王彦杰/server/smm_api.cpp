...
SMM_INT SC_accept(SMM_INT iSocket)
{
	...
	{   
        ...
        stSSLSocket[iClient_Socket].ssl = SSL_new(stSSLSocket[0].ctx) ; //创建SSL套接字
	printf("ssl_new01\n") ;

        iError = SSL_set_fd(stSSLSocket[iClient_Socket].ssl,stSSLSocket[iClient_Socket].sock_serv);
		if (iError<0)
		{
			return -5 ;
		}
        //(3-3)等待客户端ssl连接请求
		iError = SSL_accept(stSSLSocket[iClient_Socket].ssl) ;
		printf("ssl_accept=%d",iError) ; //测试SSL_accept返回值
		if (iError <0 )
		{   
			SSL_shutdown(stSSLSocket[iClient_Socket].ssl) ; 
			return -6 ;
		}
...
/********************************************************
  ...
  Others:      
***********************************************************/
SMM_INT SC_select(SMM_INT iNfds,SMM_INT iRwe,SMM_INT iBlock,fd_set *pFd)
	{
		SMM_INT  iNready = -1 ;
		timeval timeout ;
		if (iBlock == 0) //非阻塞
		{
		 ...
		 else
		 {
		 	if (iBlock >0) //阻塞一定的时间
		 	{
		            timeout.tv_sec  = iBlock ;
		            timeout.tv_usec = iBlock ; 
		        }
		}  
		...
		switch(iRwe)
		{
			case 0:    //可读
			        if (iBlock >=0)
			        {
				    iNready=select(0,pFd,NULL,NULL,&timeout);
				}
				else
				{
			...
			case 1:   //可写
			        if (iBlock >=0)
			        {
			            iNready=select(0,NULL,pFd,NULL,&timeout);
			        }
			        else
			        {
			...
			case 2:  //异常
			        if (iBlock >=0)
			        {
			        	iNready=select(0,NULL,NULL,pFd,&timeout);
			        }
			        else
			        {
...
/********************************************************
  ...
  Others: 内部使用     
***********************************************************/
SMM_INT SC_getip(SMM_INT  iSocket,char *ipaddr,SMM_USHORT *port)
    {
...
/********************************************************
  Function:       SC_setsockopt
  Description:    设置socket属性
  Calls:          
  Called By:      
  Table Accessed: 
  Table Updated:  
  Input:         SOCKET iSocket:socket套接字     
                 int    level  :1:表示socket;
		 int   optname :设置选项	   
		               1:表示设置SO_LINGER(关闭socket延时秒数)
		               2:SO_RCVBUF 接收缓冲区
		               3:SO_SNDBUF 发送缓冲区
		               
                 int   value   :设置的值			       
                               (1)当optname==1 ;value>0 延时秒 value<=0不延时
  ...
  Others:      
***********************************************************/
SMM_INT SC_setsockopt(SOCKET iSocket, int level, int optname,int value)
{
	SMM_INT isock_cli ;
	SMM_INT iReturn   ;   //返回值
	//(1)取出socket信息
	isock_cli = SC_queryarray(iSocket) ;
	if (isock_cli == -1)  //-1:没有指定的socket
	{
		return -1 ;
	}
	//(2)设置socket选项
	struct linger Linger; 
    //(2-1)设置socket关闭延时(开启socket,默认不延时,立即关闭)
    if (level == 1)   //socket
    {
		   switch (optname)   
		   {
	   			case 1:   //1:表示 Linger:设置closesocket延时
	   				  if (value <= 0)  //不延时
	   				  {
						  Linger.l_onoff = 0;   // 开启 linger 设定       
	   				  }
	   				  else             //延时value秒
	   				  {
	   	      			  Linger.l_linger = value;  //设定 linger 时间为 n 秒 
	   				  }
	   				  
	   				  iReturn = setsockopt(stSSLSocket[isock_cli].s , SOL_SOCKET, SO_LINGER, (const char *)&Linger, sizeof(struct linger) );
	   			...
	   			/*case 2:  //接收缓冲区
	   				 ...
	   				 //iReturn = setsockopt(stSSLSocket[isock_cli].s , SOL_SOCKET, SO_RCVBUF, &Linger, sizeof(struct linger) );
	   			case 3:  //发送缓冲区
					//
	   			case 4:  //发送保持时间
					// */
...
/********************************************************
  Function:       SC_GetError
  Description:    取得最近一次的错误代码
  Calls:          
  Called By:      
  Table Accessed: 
  Table Updated:  
...
/********************************************************
  Function:       SC_GetErrorString
  Description:    取得最近一次的错误说明
  Calls:          
  Called By:      
  Table Accessed: 
  Table Updated:  
