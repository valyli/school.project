...
void CCPUDlg::OnButton1() 
{
	...
	//利用汇编获取在一定时间内CPU经过的时间片
	tempcount = theCycleCount();
	Sleep(1000);
	tempcount = theCycleCount() - tempcount;
	//调用精确计时器
	QueryPerformanceCounter(&litmp);
	...
	cpuspeed = tempcount/(((QPart2 - QPart1)/(double)litmp.QuadPart)*1000000);

	strRawClockFrequency.Format("%.2f MHZ estimate ", cpuspeed);
