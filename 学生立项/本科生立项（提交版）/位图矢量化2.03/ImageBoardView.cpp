...
BEGIN_MESSAGE_MAP(CImageBoardView, CScrollView)
	...
	ON_COMMAND(IDM_PREVIEW_TRACE, OnPreviewTrace)
	//}}AFX_MSG_MAP
...
void CImageBoardView::OnFileCut() 
{
	if(!m_pDib->Result.n)
		AfxMessageBox("未取边缘无法进行切割");
	m_Plotter.OpenPort();
    m_Plotter.InitPlotter();
	tagResult* t = m_pDib->Result.next;
	while(t != 0)
	{
		m_Plotter.Shape(t->q, t->n, acc);
		t = t->next;
	}
	m_Plotter.ClosePort();
}

void CImageBoardView::OnTrace() 
{   
    acc = 1;
	
	if(!m_pDib->Trace()/*Trace(q, n)*/)
		AfxMessageBox("边缘跟踪失败！");
...
/*	int size = 3;
	...
	while(t != 0)
	{
		for(int i = 0; i < t->n; i++)
		{
			t->q[i].x = size * t->q[i].x;
			t->q[i].y = size * t->q[i].y;
		}
		t = t->next;
	}
*/
...
void CImageBoardView::PreviewTrace()
{
	CPoint temp;
	temp = GetScrollPosition();
	CDC* pDC;
	pDC = GetWindowDC();
	CPen m_Pen, *old_Pen;
	m_Pen.CreatePen(PS_SOLID, 1, RGB(0, 255, 0));
	old_Pen = pDC->SelectObject(&m_Pen);
	tagResult* t = m_pDib->Result.next;
	//调整坐标用的修正值
	int acc = 2;
	while(t != 0)
	{
		pDC->MoveTo(t->q[0].x - temp.x + acc, t->q[0].y - temp.y + acc);
		for(int i = 1; i < t->n; i++)
		{
			pDC->LineTo(t->q[i].x - temp.x + acc, t->q[i].y - temp.y + acc);
//			pDC->Ellipse(t->q[i].x - temp.x + acc - 3, t->q[i].y - temp.y + acc - 3, 
//				t->q[i].x - temp.x + acc + 3, t->q[i].y - temp.y + acc + 3);

		}
		pDC->LineTo(t->q[0].x - temp.x + acc, t->q[0].y - temp.y + acc);
		t = t->next;
	}
}

void CImageBoardView::OnPreviewTrace() 
{
	if(IsDrawPreviewTrace == false)
		IsDrawPreviewTrace = true;
	else
		IsDrawPreviewTrace = false;
	Invalidate();
}
