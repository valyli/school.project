...
//////////////////////////delete the memory used by Result
/*	if(Result.next != 0)
	{
		tagResult* t;
		t = Result.next;
...
//tagResult Result;
//BOOL CDib::Trace(Coordinates *q, int &n)
BOOL CDib::Trace()
{
/*	n = 7;
	q[0].x=100;
...
*/
	long i,j;
	AfxMessageBox("将位图格式转成二值数组",MB_OK);
	unsigned char p[800][800] = {0};
	char a[20];
	MessageBox(0, itoa(p[799][799], a, 10), "", MB_OK);
	long width, height;
	width = GetWidth();
	height = GetHeight();
	for(i = 0; i < width; i++)
	...
	do
	{
		...
		for(i = 0; i < 800; i++)
			for(j = 0; j < 800; j++)
				if(p[i][j] == 1)
					n++;
		MessageBox(0, itoa(n, a, 10), "", MB_OK);
		if(n == 0)
		{
			break;
		}
...
//	n--;
		...
		{
			EraserRect(p, q, n, 800, 800);
			if(count == 0)
			{
				t = &Result;
			}
			t->next = (tagResult*)malloc(sizeof(tagResult));
			t = t->next;
			t->q = (Coordinates*)malloc(n * sizeof(Coordinates));
			for(i = 0; i < n; i++)
				t->q[i] = q[i];
			t->n = n;
			t->next = 0;
...
//////////////////////////////////////////
	fclose(fp);
			MessageBox(0, itoa(Result.n, a, 10), "", MB_OK);
t = Result.next;
	if( IDYES == AfxMessageBox("是否对轮廓序列进行过滤，防止矢量化时出现二次抖动？",MB_YESNO))
	{
		while(t != 0)
		{
			TraceFilter(t->q, t->n);
...
/*	t = Result.next;
	if( IDYES == AfxMessageBox("是否使用后续的矢量化算法？",MB_YESNO))
	{
		while(t != 0)
		{
char a[30];
MessageBox(0, itoa(t->n, a, 10), "", MB_OK);
//			TraceVector(t->q, t->n, 1, 175.0, 10.0, 500.0, 30.0, 0);
			TraceFilter(q, n);
			t = t->next;
		}
	}
...
//以下函数用于对轮廓序列（8邻接）的过滤处理
//写于2001年8月24日

bool CDib::TraceFilter(Coordinates *q, int &n)
{
	if(n <= 0 || q == 0)
		return false;
	Coordinates temp[10000];
/*	//用于判断方向变化
	//只有-1，0，1三种状态
	int dx, dy;
	//关键点计数器
...
//////////////////////////////////////////////////////////////
//给样条曲线上的四个点，返回AC，BD所构成的角的度数的Cos值。

double GetCosA(Coordinates A, Coordinates B, Coordinates C, Coordinates D)
{
	int x, y;
	double b, c, a2, b2, c2;
	double CosA;
...
bool CDib::TraceVector(Coordinates *q, int &n, int times, double range, double step, double outstep, double repaircoe, int line_switch)
{
	if(n <= 0 || q == 0 || range < 0.0 || range >180.0 || step > outstep || repaircoe <1.0)
		return false;
	range = range * 0.01745329;
	range = cos(range);
	Coordinates temp[10000];
	double CosA;
	double oldd, d, remain, num, t;
	int x, y;
	int m;
	...
	for(int j = 1; j <= times; j++)
	{
		...
		for(int i = 1; i < n; i++)
		{
			CosA = GetCosA(temp[m-1], q[i], q[i+1], q[i+2]);
			x = q[i].x - temp[m-1].x;
			y = q[i].y - temp[m-1].y;
			oldd = sqrt((x*x)+(y*y));
			if(oldd > step)
			{
				...
				{
					remain = oldd;
					for(num = (line_switch?0:1); (remain > (2.0 * pow(repaircoe, num) * step)); num++)
						remain = remain - 2.0 * pow(repaircoe, num) * step;
					num--;
					for(t = (line_switch?0:1); t <= num; t++)
					{

						d = pow(repaircoe, t) * step;
						d = d/oldd;
						temp[m].x = temp[m-1].x + (int)(x * d);
						temp[m].y = temp[m-1].y + (int)(y * d);
						m++;
					}
					int z;
//					z = m - (int)num - 1 - line_switch;
//					temp[m].x = (int)((temp[z].x + q[i].x)/2.0);
//					temp[m].y = (int)((temp[z].y + q[i].y)/2.0);
//					z = m + (int)num + 1 + line_switch;
					z = m + (int)num + line_switch;///////new
					temp[z] = q[i];
//					m = (int)(m + num + 1 + line_switch);
					m = (int)(m + num +line_switch);/////new
					for(t = (line_switch?0:1); t <= num; t++)
					{
						z = m - (int)t - line_switch;
						d = pow(repaircoe, t) * step;
						d = d/oldd;
						temp[z].x = temp[z+1].x - (int)(x * d);
						temp[z].y = temp[z+1].y - (int)(y * d);
					}
					m++;
				}
				else if(CosA > range)
		...
		CosA = GetCosA(temp[m-1], q[0], q[1], q[2]);
		x = q[0].x - temp[m-1].x;
		y = q[0].y - temp[m-1].y;
		oldd = sqrt((x*x)+(y*y));
		...
		else if(oldd > outstep)
		{
			...
			int z;
//			z = m - (int)num - 1 - line_switch;
//			temp[m].x = (int)((temp[z].x + q[0].x)/2.0);
//			temp[m].y = (int)((temp[z].y + q[0].y)/2.0);
//			z = m + (int)num + 1 + line_switch;
			z = m + (int)num + line_switch;//////new
			temp[z] = q[i];
			m = (int)(m + num + line_switch);/////new
...
BOOL CDib::EraserRect(unsigned char p[][800], Coordinates *q, int n, int x, int y)
{
	long i,j,temp;
	for(i = 0; i < n; i++)
	{
		p[q[i].x][q[i].y] = 2;//make flag for trace
	}
	
	for(i = 0; i < x; i++)//eraser rect in trace
	{
		temp = 0;
		for(j = 0; j < y; j++)
		{
			if(p[i][j] == 2 && temp == 0)//start eraser
				temp++;
			else if(p[i][j] == 2 && p[i][j+1] == 0)
				temp = 0;//end eraser
			else if(temp == 1 && p[i][j] == 0)
				temp = 0;//end eraser rect when only one point
			else if(temp > 0 && p[i][j] == 1)//eraser rect(in)
			{
				p[i][j] = 0;//1 to 0
				temp++;
			}
...
double CDib::Derivative(Coordinates *q, int Num, int i)
{
	int Acc = 20;
	int i1, i2;
	double x2, y2;
	if(i < Acc)
		...
		i1 = i - Acc;
	i2 = (i + Acc) % Num;
	x2 = (q[i1].x - 2 * q[i].x + q[i2].x);
	y2 = (q[i1].y - 2 * q[i].y + q[i2].y);
	return x2 * x2 + y2 * y2;
...
/*	int f1, f2, n1, n2;
	...
	x2 = (16 * q[f1].x + 16 * q[n1].x - q[f2].x - 30 * q[i].x - q[n2].x) / 12;
	y2 = (16 * q[f1].y + 16 * q[n1].y - q[f2].y - 30 * q[i].y - q[n2].y) / 12;
	return x2 * x2 + y2 * y2;
*/
}
