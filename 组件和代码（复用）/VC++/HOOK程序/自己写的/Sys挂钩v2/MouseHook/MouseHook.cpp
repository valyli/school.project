...
*/		if(pCWPSTRUCT->message == WM_CREATE && ::GetWindow(glhTargetWnd, GW_OWNER) == NULL && GetParent(glhTargetWnd) == NULL)//WM_SHOWWINDOW)
		{  
			char szCaption[100];
//			::GetWindow(TargetWnd, GW_OWNER);
			...
			if(IsWindow(glhDisplayWnd1) && IsWindow(glhDisplayWnd2))  
			{
				...
				::PostMessage(::GetParent(glhDisplayWnd1), WM_HOOK_CREATEWINDOW, wparam, lparam);
			}
