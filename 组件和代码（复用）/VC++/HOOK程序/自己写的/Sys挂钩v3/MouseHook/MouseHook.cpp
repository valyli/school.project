...
//		CWPStruct &&
		...
		pCWPStruct->hwnd &&
		//选择程序将要处理的消息种类
		pCWPStruct->message == WM_SHOWWINDOW &&//WM_CREATE &&
		...
		)
	{
		...
		{
			//给输出窗口发消息，传递参数lparam
//			SendMessage(m_hReceiveWindow, WM_HOOK_CREATEWINDOW, 100, (LPARAM)"abcdef");
			::PostMessage(/*::GetParent(*/m_hReceiveWindow/*)*/, WM_HOOK_CREATEWINDOW, 100, (LPARAM)hWnd);//(LPARAM)(PCWPSTRUCT)(&CWPStruct));//(LPARAM)hWnd);
		}
	}  
