...
*/
	...
	{
		// 在此加入码段
		node<T> *plow, *pmid, *phigh, *p1, *p2, *t;
		plow = Get(low);
		pmid = Get(mid);
		phigh = Get(high);
		p1 = plow;
		p2 = pmid->next;
		while(p1 != pmid->next && p2 != phigh->next)
		{
	...
	void MergePass(int length)
	{
		// 在此加入码段
		int low, mid, high;
		for(low = 0; (low + length) <= (m_Size - 2); low = low + length)
		{
			mid = low + length - 1;
...
*//////////////////////////////////////////////////
	node<T> *p1 = m_Head->next, *p2, *p_end = m_Head;
	if(p1 == m_Head)
		return;
	while(m_Head->next != p_end)
	{
		p1 = m_Head->next;
		p2 = p1->next;
		while(p1->next != p_end)
