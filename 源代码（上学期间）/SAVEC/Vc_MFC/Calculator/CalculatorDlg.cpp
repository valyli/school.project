...
void CCalculatorDlg::OnButtonLog() 
{
	...
	{
		if(Newnum <= 0)
			MessageBox("it is too min or not int");
		else
		{
			Oldnum =log10(Newnum);
			Endnewnum();
...
unsigned long Math_X(unsigned long Num)
{
...
void CCalculatorDlg::OnButtonX() 
{
	if(Firstnum  == 1)
	{
		if(Newnum <1 || int(Newnum) != Newnum)
			MessageBox("it is too min or not int");
		else
		{
			Oldnum = double(Math_X(unsigned long(Newnum)));
			Firstnum = 0;
			Endnewnum();
			Showresult();
			Lastproc = 12;
	...
	else if(Oldnum <1 || int(Oldnum) != Oldnum)
	{
		MessageBox("it is too min or not int");
	}
	else 
	{
		Oldnum =double(Math_X(unsigned long(Oldnum)));
		Endnewnum();
		Showresult();
		Lastproc = 12;
		Equed = 0;
